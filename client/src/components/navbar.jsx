...
export default function Navbar() {
    ...
    return (
        <nav className="bg-[#006769] p-6 zindex">
                ...
                <div className="relative flex items-center justify-between h-16">
                    ...
                    <div className="hidden sm:block sm:ml-6">
                        <div className="flex space-x-4">
                            <Link to='/novopedido'>
                                <button onClick={CriarNovoPedido} className="bg-[#9DDE8B] hover:bg-[#E6FF94] text-white font-bold py-2 px-4 rounded">
                                    Novo Pedido
                                </button>
                            </Link>
                            {ativoAdm && (
                                <Link to='/lista'>
                                    <button className="bg-[#9DDE8B] hover:bg-[#E6FF94] text-white font-bold py-2 px-4 rounded">
                                        Lista de usuários
                                    </button>
                                </Link>
                            )}
                            {ativoAdm && (
                                <Link to='/financeiro'>
                                    <button className="bg-[#9DDE8B] hover:bg-[#E6FF94] text-white font-bold py-2 px-5 rounded">
                                        Financeiro
                                    </button>
                                </Link>
                            )}
            ...
            {/* Menu responsivo */}
            <div className={`sm:hidden ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} absolute top-0 left-0 h-screen w-1/2 bg-[#006769] transition-transform duration-300 ease-in-out`}>
                <div className="p-4">
                    ...
                    <div className="mt-6 space-y-4 ">
                        {/* Botão Novo Pedido */}
                        <Link to='/novopedido'>
                            <button onClick={CriarNovoPedido} className="block w-full px-4 py-2 mb-4 text-center text-sm font-medium text-white bg-[#9DDE8B] rounded-md hover:bg-[#E6FF94]">
                                Novo Pedido
                            </button>
                        </Link>
                        {/* Lista de usuários */}
                        {ativoAdm && (
                            <Link to='/lista'>
                                <button className="block w-full px-4 py-2 text-center text-sm font-medium text-white bg-[#9DDE8B] rounded-md hover:bg-[#E6FF94] mb-2">
                                    Lista de usuários
                                </button>
                            </Link>
                        )}
