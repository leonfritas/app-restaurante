...
export default function ListUser() {
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      <Link to="/cadastro">
        <button className=" m-4 text-white w-60 py-3 my-2 leading-none bg-indigo-600 hover:bg-indigo-700 font-semibold rounded shadow">
        ...
        {listUser.length > 0 ? (
          listUser.map((user, index) => (
            <li key={index} className="bg-slate-700 p-4 rounded-lg shadow-md">
              ...
              <p className="text-white font-mono">
                Funcionario: {user.ativoFuncionario?.data[0]}
              </p>
              <p className="text-white font-mono">
                Administrador: {user.ativoAdministrador?.data[0]}
              </p>
              <button
                className="leading-1 w-35 py-1 rounded shadow buttonDeleteUser text-white bg-red-300"
                onClick={() => deleteUser(user.idFuncionario)}
              >
                EXCLUIR FUNCIONÁRIO
              </button>
            </li>
