...
export async function getTable(action, idGrupoPedido) {
  try {
    if (action === "disponiveis") {
      const response = await Axios.post("https://c630c4e6c2fd.ngrok-free.app/table/getTable", {
        database: sessionStorage.getItem("database"),
      });
      return response.data;
    } else if (action === "ocupadas") {
      const response = await Axios.post("https://c630c4e6c2fd.ngrok-free.app/table/getOrderTable", {
  ...
  export async function joinTable(idGrupoPedido, idMesa){  
    if (idGrupoPedido > 0) {      
        await Axios.post("https://c630c4e6c2fd.ngrok-free.app/table/joinTable", {          
          idMesa: idMesa,
          idGrupoPedido: idGrupoPedido,
          database: sessionStorage.getItem("database")
        });
